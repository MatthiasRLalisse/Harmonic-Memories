...
class KBEHoldOutTask(KBETaskSetting):
	...
	def build_Graph(self, include_heldout=False, include_valid=True):
	...
	def rankEntities(self, model, entity_1s,relations_,entity_2s, direction='r', \
		...
		return ranked
	def eval(self, model, sess=None, test_set=False, interactive=False, num_to_test=0, full_graph=False, data_key=None):
		if not sess: sess = model.sess
		...
		for k in range(test_batches_):
			...
			try:
				...
				mean_rank_e2 = int(np.mean(left_rank))
