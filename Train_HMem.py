...
task = gg.KBETaskSetting(dataName=DATA, negsamples=NEGSAMPLES, batch_size=BATCHSIZE, \
...
for e in range(1, MAXEPOCH+1):	
	...
	if e % TEST_EVERY == 0: 
		results_valid = task.eval(model, interactive=INTERACTIVE, num_to_test=NUM_TO_TEST) 	#evaluate on validation set
...
test_model = gg.models.HHolE(task=task, entity_dim=DIM, \
